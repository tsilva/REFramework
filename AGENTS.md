...
Use `dev/build-dev.ps1` for local and release-candidate builds. This is our stable build entry point.

Required local prerequisites:

- Visual Studio 2022 C++ build tools.
- CMake, either on `PATH` or bundled with Visual Studio 2022.
- .NET Framework 4.7.2 Developer Pack/Targeting Pack.

```powershell
cd C:\Users\engti\Desktop\REFramework-clean
powershell -ExecutionPolicy Bypass -File .\dev\build-dev.ps1
```

The script always uses the repo-root `build/` directory, which is ignored by Git. It does not run `git pull`, so it will only build the commit currently checked out. It initializes submodules to the exact commits recorded by the repo and writes `src/CommitHash.autogenerated` from Git metadata instead of the wall clock, so repeated builds of the same commit use the same version header.

Useful options:

```powershell
powershell -ExecutionPolicy Bypass -File .\dev\build-dev.ps1 -Clean
powershell -ExecutionPolicy Bypass -File .\dev\build-dev.ps1 -Configuration RelWithDebInfo
powershell -ExecutionPolicy Bypass -File .\dev\build-dev.ps1 -Target REFramework
```

By default, the script refuses to build a dirty tree. Use `-AllowDirty` only for throwaway local experiments, not release builds.

The root `build_vs2022.bat` and `build_vs2022_noautogen.bat` files delegate to this script for compatibility. They should not grow their own build folders or run `git pull`.
